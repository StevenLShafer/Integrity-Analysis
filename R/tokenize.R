...
#   medianRng 45 [30-60]    |  45 (30-60)  |  45 (30 to 60)   -> unusable
#   nPct      15 (60%)      |  15 (60.0 %)
...
.ppTokenRegex <- local({
  ...
  paste0(
    "(?<![A-Za-z0-9_.])(?:",
    "(?<meanSD>",    NUM, "\\s*(?:\u00b1|\\+/-|\\+-)\\s*", NUM, ")",
    # range separator: hyphen, en/em dash, Unicode minus (U+2212 - what
    # PDF fonts often use for "-"), or the word "to"
...
.ppTokenizeLine <- function(line) {
  ...
  tokens <- lapply(seq_along(starts), function(i) {
    ...
    nums    <- regmatches(tokText, gregexpr(.ppNUM, tokText, perl = TRUE))[[1]]
